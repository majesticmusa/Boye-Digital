...
export default function App() {
    ...
    const [conversation, setConversation] = useState<{
        ...
        history: [{id: 0, sender: 'system', text: 'Your practice session has started. Speak into your microphone.'}],
        currentInput: '',
        currentOutput: '',
    });
    ...
    const startSession = useCallback(async () => {
        ...
        setConversation({ history: [{id: 0, sender: 'system', text: 'Your practice session has started. Speak into your microphone.'}], currentInput: '', currentOutput: '' });
        ...
        try {
            ...
            sessionPromiseRef.current = aiRef.current.live.connect({
                ...
                config: {
                    ...
                    systemInstruction: 'You are a friendly and encouraging public speaking coach. Your goal is to help the user practice their vocal cadence. Engage in a natural conversation, provide feedback, and offer tips on pacing, pausing, and emphasis.',
                },
                callbacks: {
                    onopen: () => {
                        setIsConnecting(false);
    ...
    const getStatus = () => {
        ...
        if (!isSessionActive) return { text: 'Inactive', color: 'gray' };
    ...
    const showChat = isSessionActive || isConnecting;
        ...
        <div className="flex flex-col h-screen bg-sky-950 text-white">
            <header className="flex-shrink-0 bg-sky-900/50 backdrop-blur-sm shadow-md p-4 flex justify-between items-center z-10">
                ...
                <div className="flex items-center space-x-2">
                    <span className={`relative flex h-3 w-3`}>
                        <span className={`${isSessionActive ? 'animate-ping' : ''} absolute inline-flex h-full w-full rounded-full bg-${status.color}-400 opacity-75`}></span>
            ...
            <main className="flex-1 overflow-y-auto pb-32">
                 {showChat ? (
                    ...
                    activeTab === 'lessons' ? <Lessons ai={aiRef.current} /> : <Drills />
                )}
            </main>
            ...
            <footer className="fixed bottom-0 left-0 right-0 p-4 flex justify-center bg-gradient-to-t from-sky-950 to-transparent">
                <button
                    ...
                    aria-label={isSessionActive ? 'Stop session' : 'Start practice session'}
                    className={`relative w-20 h-20 rounded-full text-white shadow-lg transition-all duration-300 ease-in-out transform hover:scale-105 flex items-center justify-center
                        ${isConnecting ? 'bg-amber-500 cursor-not-allowed animate-pulse' : isSessionActive ? 'bg-red-600 hover:bg-red-700' : 'bg-amber-600 hover:bg-amber-700'}`}
                >
                    {isConnecting ? (
